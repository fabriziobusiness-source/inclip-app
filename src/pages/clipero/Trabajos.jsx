...
export default function Trabajos() {
  ...
  const { datos, cargando, error } = useDatos(async () => {
    ...
    return supabase
      .from('trabajos')
      .select('*, postulaciones(id, clipero_id)')
      .eq('estado', 'abierto')
    ...
    <>
      ...
      {cargando ? (
        ...
        <EmptyState
          ...
          accion={datos?.length ? undefined : 'Completar mi perfil'}
          accionTo="/clipero/perfil"
        />
      ) : (
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {trabajos.map((t) => {
              const yaOferte = (t.postulaciones || []).some((p) => p.clipero_id === usuario.id);
              return (
                <TrabajoCard
                  key={t.id}
                  trabajo={t}
                  to={`/clipero/trabajos/${t.id}`}
                  pie={
                    yaOferte ? (
